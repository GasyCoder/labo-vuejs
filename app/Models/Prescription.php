...
class Prescription extends Model
{
    ...
    public function getMontantPrelevementsCalcule()
    {
        return $this->prelevementsAvecQuantite()->sum(function ($prelevement) {
            // Utiliser la méthode du modèle Prelevement si possible
            if ($prelevement->quantite_tubes > 1 && $prelevement->prix_promotion > 0) {
                return $prelevement->prix_promotion * $prelevement->quantite_tubes;
            }
    ...
    public function getMontantTotalAttribute()
    {
        ...
        if ($this->patient_type === 'URGENCE-NUIT') {
            $total += Setting::getTarifUrgenceNuit();
        } elseif ($this->patient_type === 'URGENCE-JOUR') {
            $total += Setting::getTarifUrgenceJour();
        }
