...
class Prelevement extends Model
{
    ...
    /**
     * Scope pour rechercher par dénomination
     */
    public function scopeRecherche($query, $terme)
    {
        return $query->where('denomination', 'like', "%{$terme}%");
    }
    ...
    private function getTypeTubeFallback()
    {
        ...
        if ($this->estSanguin()) {
            ...
            if (
                stripos($this->denomination, 'HGPO') !== false ||
     ...
     * Utilise le prix de promotion si quantité > 1
     */
    public function calculerPrixTotal($quantite = 1)
    {
        if ($quantite > 1 && $this->prix_promotion > 0 && $this->prix_promotion <= $this->prix) {
            return $this->prix_promotion * $quantite;
        }
