...
class ResultatPdfShow
{
    ...
    private function generatePDF(Prescription $prescription, $examens, $type = 'final')
    {
        ...
        $data = [
            ...
            'notesByAnalyse' => AnalyseConclusionNote::where('prescription_id', $prescription->id)
                ->orderBy('created_at')
                ->get()
                ->groupBy('analyse_id'),
            'groupConclusions' => PrescriptionExamenConclusion::where('prescription_id', $prescription->id)
                ->get()
                ->keyBy('examen_id'),
        ];
