        ...
        <div class="flex flex-col w-full h-[calc(100vh-3.5rem)]">
            <div class="h-full pt-3 pb-8" data-simplebar>
                <ul class="nk-menu">
                            ...
                            <a href="{{ route('admin.gestion-analyses') }}"
                                ...
                                <span
                                    ...
                                    Suivi des Analyses (<span
                                        id="analyses-count">{{ \App\Models\Prescription::where('status', '!=', \App\Models\Prescription::STATUS_ARCHIVE)->count() }}</span>)
                                </span>
                            </a>
                        </li>
                    @endif

                    @if(auth()->check() && auth()->user()->hasAnyPermission(['prescriptions.voir', 'analyses.voir', 'patients.voir', 'prescripteurs.voir']))
                        {{-- Section Secrétaire / Gestion Prescriptions --}}
                        @if(auth()->user()->hasPermission('prescriptions.voir'))
                            <li
                                class="relative first:pt-1 pt-6 pb-1 px-4 before:absolute before:h-px before:w-full before:start-0 before:top-1/2 before:bg-gray-200 dark:before:bg-gray-900 first:before:hidden before:opacity-0 group-[&.is-compact:not(.has-hover)]/sidebar:before:opacity-100">
                                <h6
                                    class="group-[&.is-compact:not(.has-hover)]/sidebar:opacity-0 text-slate-400 dark:text-slate-300 whitespace-nowrap uppercase font-bold text-xs tracking-relaxed leading-tight">
                            ...
                            <li
                                class="nk-menu-item py-0{{ request()->routeIs('secretaire.journal-decaissement') ? ' active' : '' }} group/item">
                                <a href="{{ route('secretaire.journal-decaissement') }}"
                                    class="nk-menu-link flex relative items-center align-middle py-2 ps-5 pe-8 font-heading font-bold tracking-snug group">
                                    <span
                                        class="font-normal tracking-normal w-8 inline-flex flex-grow-0 flex-shrink-0 text-slate-400 group-[.active]/item:text-primary-500 group-hover:text-primary-500">
                                        <em
                        ...
                        @if(auth()->user()->hasPermission('patients.voir'))
                            <li
                                class="nk-menu-item py-0{{ request()->routeIs('secretaire.patients.index', 'secretaire.patient.detail') ? ' active' : '' }} group/item">
                        ...
                        @if(auth()->user()->hasPermission('prescripteurs.voir'))
                            <li
                                class="nk-menu-item py-0{{ request()->routeIs('secretaire.prescripteurs') ? ' active' : '' }} group/item">
                                <a href="{{ route('secretaire.prescripteurs.index') }}"
                                    class="nk-menu-link flex relative items-center align-middle py-2 ps-5 pe-8 font-heading font-bold tracking-snug group">
                                    <span
                                        class="font-normal tracking-normal w-8 inline-flex flex-grow-0 flex-shrink-0 text-slate-400 group-[.active]/item:text-primary-500 group-hover:text-primary-500">
                                        <em
                    ...
                    <!-- Technicien -->
                    @if(auth()->check() && auth()->user()->hasPermission('analyses.effectuer'))
                        <li
                    ...
                    <!-- Biologiste -->
                    @if(auth()->check() && auth()->user()->hasPermission('analyses.valider'))
                        <li
                    ...
                    <!-- Archives -->
                    @if(auth()->check() && auth()->user()->hasPermission('archives.acceder'))
                    ...
                    {{-- Section Laboratoire --}}
                    @if(auth()->check() && (auth()->user()->isAdmin() || auth()->user()->hasPermission('laboratoire.gerer')))
                        <li
                            class="relative first:pt-1 pt-6 pb-1 px-4 before:absolute before:h-px before:w-full before:start-0 before:top-1/2 before:bg-gray-200 dark:before:bg-gray-900 first:before:hidden before:opacity-0 group-[&.is-compact:not(.has-hover)]/sidebar:before:opacity-100">
                            <h6
                                class="group-[&.is-compact:not(.has-hover)]/sidebar:opacity-0 text-slate-400 dark:text-slate-300 whitespace-nowrap uppercase font-bold text-xs tracking-relaxed leading-tight">
