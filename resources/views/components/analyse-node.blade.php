...
@php
    ...
    $notesForAnalyse = $notes[$node->id] ?? [];
    $editingId = $noteEditId[$node->id] ?? null;
    $editingText = $noteEditText[$node->id] ?? '';
    $saveStatus = $autoSaveStatus[$node->id] ?? null;
    $saveAt = $autoSaveAt[$node->id] ?? null;
    $saveError = $autoSaveError[$node->id] ?? null;
@endphp
            ...
            <div class="flex items-center gap-3 mb-2">
                ...
                @if($saveStatus)
                    <span class="text-[11px] px-2 py-0.5 rounded-full border
                        {{ $saveStatus === 'saving' ? 'border-amber-200 bg-amber-50 text-amber-700 dark:bg-amber-900/20 dark:text-amber-300 dark:border-amber-800' : '' }}
                        {{ $saveStatus === 'saved' ? 'border-emerald-200 bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-300 dark:border-emerald-800' : '' }}
                        {{ $saveStatus === 'error' ? 'border-red-200 bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-300 dark:border-red-800' : '' }}
                    ">
                        @if($saveStatus === 'saving')
                            Enregistrement…
                        @elseif($saveStatus === 'saved')
                            Sauvegardé {{ $saveAt ? 'à ' . $saveAt : '' }}
        ...
        <div class="space-y-4 pl-4 border-l-2 border-slate-200 dark:border-slate-700">
            @foreach($node->enfantsRecursive as $child)
                <x-analyse-node
                    ...
                    :patient="$patient"
                    :notes="$notes"
                    :note-edit-id="$noteEditId"
                    :note-edit-text="$noteEditText"
                    :can-edit-conclusions="$canEditConclusions"
                    :auto-save-status="$autoSaveStatus"
                    :auto-save-at="$autoSaveAt"
    ...
    @else
        ...
        <div class="space-y-4">
                ...
                @case('COMPTAGE')
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        <div class="space-y-2">
                            ...
                            <input type="text" wire:model.live.debounce.1000ms="{{ $path }}.valeur"
                        ...
                        <div class="bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg p-4 max-h-48 overflow-y-auto space-y-2">
                            ...
                            @if(is_array($get('resultats')) && in_array('Autre', $get('resultats')))
                                <div class="mt-3 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700">
                                    ...
                                    <textarea rows="2" wire:model.live.debounce.1000ms="{{ $path }}.valeurAutre"
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-2">
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">
                                    ...
                                    <input type="text" wire:model.live.debounce.1000ms="{{ $path }}.resultats"
                                   class="w-full px-4 py-2.5 bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:ring-2 focus:ring-primary-500 dark:focus:ring-primary-400 focus:border-primary-500 dark:focus:border-primary-400 transition-colors"
                    ...
                    <div class="bg-gradient-to-r from-pink-50 to-pink-100 dark:from-pink-900/20 dark:to-pink-800/20 border border-pink-200 dark:border-pink-800 rounded-xl p-6">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            <div class="space-y-2">
                                ...
                                @else
                                    <textarea rows="3" wire:model.live.debounce.1000ms="{{ $path }}.resultats"
                        ...
                        @if($get('resultats') === 'Autre')
                            <div class="mt-4 p-3 bg-white dark:bg-slate-800 rounded-lg border border-pink-200 dark:border-pink-700">
                                ...
                                <textarea rows="3" wire:model.live.debounce.1000ms="{{ $path }}.valeur"
                                          class="w-full px-4 py-3 bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 resize-none focus:ring-2 focus:ring-pink-500 dark:focus:ring-pink-400 focus:border-pink-500 dark:focus:border-pink-400 transition-colors"
                ...
                @default
                    <div class="space-y-2">
                        ...
                        @else
                            ...
                            <textarea rows="4" wire:model.live.debounce.1000ms="{{ $path }}.resultats"
                                      class="w-full px-4 py-3 bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 resize-none focus:ring-2 focus:ring-primary-500 dark:focus:ring-primary-400 focus:border-primary-500 dark:focus:border-primary-400 transition-colors"
            ...
            @if(!in_array($type, ['LABEL','MULTIPLE','MULTIPLE_SELECTIF']))
                <div
                    x-data="{
                        openConclusion: {{ !empty($notesForAnalyse) ? 'true' : 'false' }},
                        showAdd: false
                    }"
                    class="pt-4 border-t border-slate-200 dark:border-slate-700 space-y-3"
                >
                    <button type="button"
                            @click="openConclusion = !openConclusion"
                            class="w-full flex items-center justify-between text-left text-sm font-medium text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-colors">
                        <span>Conclusion (optionnelle)</span>
                        <span class="inline-flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                            @if(!empty($notesForAnalyse))
                                <span class="px-2 py-0.5 bg-slate-100 dark:bg-slate-800 rounded-full">{{ count($notesForAnalyse) }} note{{ count($notesForAnalyse) > 1 ? 's' : '' }}</span>
                            @endif
                            <svg class="w-4 h-4 transition-transform" :class="openConclusion ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                            </svg>
                        </span>
                    </button>

                    <div x-show="openConclusion" x-transition class="space-y-3">
                        {{-- Notes partielles --}}
                        <div class="space-y-2">
                            <div class="flex items-center justify-between">
                                <span class="text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wide">Notes partielles</span>
                                @if($canEditConclusions)
                                    <button type="button"
                                            @click="showAdd = !showAdd"
                                            class="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium text-primary-700 dark:text-primary-300 bg-primary-50 dark:bg-primary-900/20 border border-primary-200 dark:border-primary-800 rounded-full hover:bg-primary-100 dark:hover:bg-primary-900/40 transition-colors">
                            ...
                            @if($canEditConclusions)
                                <div x-show="showAdd" x-transition class="p-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg space-y-2">
                                    <textarea rows="2"
                                              wire:model.defer="noteDrafts.{{ $node->id }}"
                                              wire:keydown.enter.prevent="addConclusionNote({{ $node->id }})"
                                              class="w-full px-3 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-md text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 resize-none focus:ring-2 focus:ring-primary-500 dark:focus:ring-primary-400 focus:border-primary-500 dark:focus:border-primary-400 transition-colors"
                                              placeholder="Note / conclusion partielle..."></textarea>
                                    <div class="flex items-center justify-end gap-2">
                                        <button type="button"
                                                @click="showAdd = false"
                                                class="text-xs text-slate-500 dark:text-slate-400 hover:text-slate-700">Annuler</button>
                                        <button type="button"
                                                wire:click="addConclusionNote({{ $node->id }})"
                                                class="px-3 py-1.5 text-xs font-semibold text-white bg-primary-600 hover:bg-primary-700 rounded-md transition-colors">
                                            Ajouter
                                        </button>
                                    </div>
                                </div>
                            ...
                            @if(!empty($notesForAnalyse))
                                <div class="space-y-2">
                                    @foreach($notesForAnalyse as $note)
                                        <div class="p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg">
                                            <div class="flex items-start justify-between gap-2">
                                                ...
                                                @if($canEditConclusions)
                                                    <div class="flex items-center gap-2">
                                                        <button type="button"
                                                                wire:click="startEditConclusionNote({{ $node->id }}, {{ $note['id'] }})"
                                                                class="text-xs text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200">
                                                            Modifier
                                                        </button>
                                                        <button type="button"
                                                                wire:click="deleteConclusionNote({{ $node->id }}, {{ $note['id'] }})"
                                            ...
                                            @if($canEditConclusions && $editingId === $note['id'])
                                                <div class="mt-2 space-y-2">
                                                    <textarea rows="2"
                                                              wire:model.defer="noteEditText.{{ $node->id }}"
                                                              class="w-full px-3 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-md text-sm text-slate-900 dark:text-slate-100 resize-none focus:ring-2 focus:ring-primary-500 dark:focus:ring-primary-400 focus:border-primary-500 dark:focus:border-primary-400 transition-colors"></textarea>
                                                    <div class="flex items-center justify-end gap-2">
                                                        <button type="button"
                                                                wire:click="cancelEditConclusionNote({{ $node->id }})"
                                                                class="text-xs text-slate-500 dark:text-slate-400 hover:text-slate-700">Annuler</button>
                                                        <button type="button"
                                                                wire:click="updateConclusionNote({{ $node->id }})"
                                                                class="px-3 py-1.5 text-xs font-semibold text-white bg-primary-600 hover:bg-primary-700 rounded-md transition-colors">
                                                            Enregistrer
                                                        </button>
                                                    </div>
                                            ...
                                            @else
                                                <div class="mt-1 text-sm text-slate-700 dark:text-slate-200 whitespace-pre-line">{{ $note['note'] }}</div>
                                            @endif
                                        </div>
                                    @endforeach
                                </div>
                            @else
                                <p class="text-xs text-slate-400 dark:text-slate-500">Aucune note pour cette analyse.</p>
                            @endif
                        </div>
