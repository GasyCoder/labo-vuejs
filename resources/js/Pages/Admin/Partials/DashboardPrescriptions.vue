...
<div>
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
        <div>
            <h3 class="text-xl font-bold text-gray-900 dark:text-white flex items-center">
                <span class="w-8 h-8 rounded-lg bg-amber-100 dark:bg-amber-900/40 text-amber-600 dark:text-amber-400 flex items-center justify-center mr-3">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                </span>
                Suivi des Prescriptions
            </h3>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1 ml-11">Vérification financière — contrôle anti-vol</p>
        </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
            ...
            <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">{{ prescriptions.total || 0 }}</p>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    <tr v-for="p in prescriptions.data" :key="p.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
...
const form = ref({
    prescriptionSearch: props.filters.prescriptionSearch || '',
    date_from: props.filters.date_from || '',
    date_to: props.filters.date_to || '',
    secretaire_id: props.filters.secretaire_id || '',
    prescriptionsPerPage: props.filters.prescriptionsPerPage || 15,
});
...
const totalMontant = computed(() => (props.prescriptions.data || []).reduce((s, p) => s + getMontantTotal(p), 0));
const totalPaye = computed(() => (props.prescriptions.data || []).reduce((s, p) => s + getMontantPaye(p), 0));
