...
<AppLayout>
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 max-w-md lg:max-w-none">
                ...
                <div
                    ...
                    <div class="text-xl font-bold text-blue-800 dark:text-blue-300">{{ stats.mois_actuel || 0 }}</div>
                </div>
                <div
                    ...
                    <div class="text-xl font-bold text-green-800 dark:text-green-300">{{ stats.total_analyses || 0 }}</div>
                </div>
                <div
                    ...
                    <div class="text-xs font-medium text-orange-600 dark:text-orange-400 uppercase tracking-wide">
                        Patients</div>
                    <div class="text-xl font-bold text-orange-800 dark:text-orange-300">{{ stats.patients_uniques || 0 }}
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="container mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                ...
                <div class="lg:col-span-2">
                    ...
                    <div class="relative">
                        <input v-model="form.search" type="text" id="search"
                ...
                <div>
                    ...
                    <select v-model="form.prescripteurFilter" id="prescripteurFilter" @change="applyFilters"
                ...
                <div>
                    ...
                    <select v-model="form.dateFilter" id="dateFilter" @change="applyFilters"
                        class="w-full py-3 px-4 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-xl text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 dark:focus:ring-primary-400 focus:border-primary-500 dark:focus:border-primary-400 transition-all">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-lg shadow overflow-hidden">
                            ...
                            <tr v-for="prescription in prescriptions.data" :key="prescription.id"
                                        ...
                                        <div class="flex-shrink-0 w-10 h-10">
                                            <div
                                                ...
                                                <span class="text-white font-bold text-sm">
                                                    {{ getInitials(prescription.patient?.nom, prescription.patient?.prenom) }}
                                                </span>
                                            </div>
                                        </div>
                                    ...
                                    <div class="flex items-center gap-3">
                                        <div class="flex-shrink-0 w-10 h-10">
                                            <div
                                                ...
                                                <span class="text-white font-bold text-sm">
                                                    {{ getInitials(prescription.prescripteur?.nom, prescription.prescripteur?.prenom) }}
                                                </span>
                                            </div>
                                        </div>
                                        <div class="flex flex-col">
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        ...
                                        {{ prescription.analyses?.length || 0 }} analyse(s)
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        ...
                                        <span class="text-xs text-slate-500 dark:text-slate-400">
                                            {{ timeAgo(prescription.updated_at) }}
                                        </span>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex justify-center gap-2">
                                        <button v-if="hasPermission('archives.restaurer')" @click="confirmUnarchive(prescription.id)"
                                        ...
                                        <button v-if="hasPermission('archives.voir')" @click="viewPrescription(prescription.id)"
                                        ...
                                        <button v-if="hasPermission('archives.supprimer')" @click="confirmPermanentDelete(prescription.id)"
                                ...
                                <td colspan="6" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-sm text-gray-500 dark:text-gray-400 max-w-sm">
                                            <template v-if="form.search || form.prescripteurFilter || form.dateFilter">
            ...
            <!-- Pagination -->
            <div v-if="prescriptions.links && prescriptions.links.length > 3" class="px-6 py-4 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between">
                    <div class="text-sm text-gray-500 dark:text-gray-400">
                        Affichage de {{ prescriptions.from }} à {{ prescriptions.to }}
                        sur {{ prescriptions.total }} résultats
                    </div>
                    <div class="flex space-x-1">
                        <template v-for="(link, key) in prescriptions.links" :key="key">
                            <div v-if="link.url === null" 
                                class="px-3 py-1 font-medium text-xs rounded-md bg-white border border-gray-300 text-gray-400" 
                                v-html="link.label">
                            </div>
                            <Link v-else :href="link.url"
                                class="px-3 py-1 font-medium text-xs rounded-md border"
                                :class="{ 'bg-primary-500 text-white border-primary-500': link.active, 'bg-white border-gray-300 text-gray-600 hover:bg-gray-50': !link.active }"
                                v-html="link.label">
                            </Link>
                        </template>
                    </div>
        ...
        <div v-if="showUnarchiveModal" class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity z-[9999]" @click="resetModal">
        ...
        <div v-if="showDeleteModal" class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity z-[9999]" @click="resetModal">
            <div class="fixed inset-0 z-[9999] w-screen">
                <div class="flex min-h-full items-center justify-center p-4"
                    style="margin-top: 4rem; padding-top: 2rem; padding-bottom: 4rem;">
                    <div class="relative transform overflow-hidden rounded-lg bg-white dark:bg-gray-800 text-left shadow-xl transition-all w-full max-w-md"
                        ...
                        <div
                            ...
                            <button @click="permanentDelete" :disabled="destroyForm.processing"
                                class="px-4 py-2 bg-red-600 hover:bg-red-700 border border-transparent rounded-md shadow-sm text-sm font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 dark:focus:ring-offset-gray-800 transition-colors disabled:opacity-50">
                                Supprimer définitivement
                            </button>
                        </div>
                    </div>
...
const form = useForm({
    ...
    dateFilter: props.filters.dateFilter || '',
    prescripteurFilter: props.filters.prescripteurFilter || '',
...
const unarchive = () => {
    unarchiveForm.post(route('archives.unarchive', selectedPrescriptionId.value), {
        onSuccess: () => resetModal()
    });
};

const confirmPermanentDelete = (id) => {
    if (!hasPermission('archives.acceder')) {
        alert('Seuls les administrateurs peuvent supprimer définitivement.');
        return;
    }
    selectedPrescriptionId.value = id;
...
const permanentDelete = () => {
    destroyForm.delete(route('archives.destroy', selectedPrescriptionId.value), {
        onSuccess: () => resetModal()
    });
};
...
const getInitials = (nom, prenom) => {
    ...
    const p = (prenom || 'A').charAt(0).toUpperCase();
    return `${n}${p}`;
};
